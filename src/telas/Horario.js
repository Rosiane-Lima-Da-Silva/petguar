...
export function Horario({ navigation }) {
  ...
  useEffect(() => {
    const loadAlarms = async () => {
      try {
        ...
        console.log('Erro ao carregar os alarmes:', error);
      }
    };
  ...
  // F salvar os alarmes no AsyncStorage
  const saveAlarms = async (updatedAlarms) => {
    try {
      await AsyncStorage.setItem('alarms', JSON.stringify(updatedAlarms));
      setAlarms(updatedAlarms);
    } catch (error) {
      console.log('Erro ao salvar os alarmes:', error);
    }
  };

  // F formato de hora (24h - HH:MM)
  const validarHora = (hora) => {
    const regex = /^([01]\d|2[0-3]):([0-5]\d)$/;
    return regex.test(hora);
  };

  // F adicionar um novo alarme
  const addAlarm = () => {
  ...
  // F atualizar o estado (ativado/desativado) de um alarme
    ...
    <View style={styles.container}>
      ...
      <View style={styles.newAlarmContainer}>
        ...
        <TouchableOpacity style={styles.addButton} onPress={addAlarm}>
          <Text style={styles.addButtonText}>Adicionar Alimentação </Text>
        </TouchableOpacity>
      </View>
